...
# def check_param(func_name, ctypes_args, real_args):
#     valid_args = [x for x in ctypes_args if not isinstance(x, ParamOut)]
#     if not len(valid_args) == len(real_args):
#         raise ValueError(
#             'Func Name: %s, Ctypes Params:%s, Real Input Params:%s' % (
#                 func_name, str(valid_args), str(real_args)))
#     return True
...
def ctypes_param(*param_args):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            self, *param = args
            # if check_param(func_name, param_args, param) is False:
            #     return
...
# def ctypes_param_check(class_obj, *args, **kwargs):
#     instance = class_obj(*args, **kwargs)
#     funcs = [x for x in dir(instance) if x.startswith('CHR')]
#     for item in funcs:
#         func = getattr(instance, item)
#         func_args = tuple([v() for k, v in func.__annotations__.items()])
#         func(*func_args)
